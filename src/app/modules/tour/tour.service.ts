...
const updateTour = async (id: string, payload: Partial<ITour>) => {
    ...
    if (payload.deleteImages && payload.deleteImages.length > 0 && existingTour.images && existingTour.images.length > 0) {
        const deleteImagesArray = Array.isArray(payload.deleteImages) ? payload.deleteImages : [payload.deleteImages];
        await Promise.all(deleteImagesArray.map((url: any) => deleteImageFromCLoudinary(url)))
    }
